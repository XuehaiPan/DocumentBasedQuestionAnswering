...
import numpy as np
import keras
from config import PREDICT_RESULTS_DIR, BATCH_SIZE, WORKERS
...
def predict() -> None:
    ...
    for epoch, model_path in enumerate(model_paths, start = 1):
        ...
        validation_file: str = os.path.join(PREDICT_RESULTS_DIR, f'validation_score_epoch{epoch}.txt')
        test_file: str = os.path.join(PREDICT_RESULTS_DIR, f'test_score_epoch{epoch}.txt')
        ...
        if model_path in top5_model_paths:
            ...
            if not os.path.exists(test_file):
                ...
                test_predictions = load_predictions(filename = test_file)
    ...
    validation_file: str = os.path.join(PREDICT_RESULTS_DIR, f'validation_score_ensemble.txt')
    test_file: str = os.path.join(PREDICT_RESULTS_DIR, f'test_score_ensemble.txt')
    save_predictions(predictions = validation_predictions, filename = validation_file)
    save_predictions(predictions = test_predictions, filename = test_file)
